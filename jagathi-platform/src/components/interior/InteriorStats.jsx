...
export default function InteriorStats() {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnter: () => {
        gsap.to(statVal1.current, {
          ...
          onUpdate: () => {
            if (textRef1.current) {
              textRef1.current.innerText = Math.floor(statVal1.current.val).toLocaleString() + 'sqft';
            }
          }
        });
        ...
        gsap.to(statVal2.current, {
          ...
          onUpdate: () => {
            if (textRef2.current) {
              textRef2.current.innerText = Math.floor(statVal2.current.val) + '+';
            }
          }
        });
  ...
  const statsData = [
    { ref: textRef1, initial: '0sqft', label: '// SQFT_FINISHED_SPACE', desc: 'Finished spatial area delivered' },
    { ref: textRef2, initial: '0+', label: '// JOINERY_UNITS', desc: 'Bespoke custom items crafted' },
    { ref: textRef3, initial: '0%', label: '// MAT_COMPLIANCE', desc: 'Acoustic & statutory standards met' },
    { ref: textRef4, initial: '0', label: '// ACTIVE_PILLARS', desc: 'Active high-profile fit-outs' }
  ];

  return (
    <section id="interior-proof" className="relative z-10 w-full bg-[#FFEA0A] border-b border-[#121315]/10 text-[#121315]">
